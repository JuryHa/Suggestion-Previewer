...
function parseCSV(csvText) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (fields.length !== headers.length) {
            fields = line.split(delimiter).map((f) => f.trim().replace(/^["'](.*)["']$/, "$1"));
        }
...
function resetOutput() {
    ...
    outputContainer.innerHTML =
        '<div class="placeholder">Enter valid JSON or CSV data on the left to see the list view here</div>';
...
function renderOutput() {
    ...
    parsedData.forEach((item, index) => {
        ...
        itemElement.style.position = "relative"; // For absolute positioning of tags and actions

        // Create driver tags container
        const driverTagsContainer = document.createElement("div");
        driverTagsContainer.className = "driver-tags-container";

        // Add driver tags if they exist
        ["driver_tag1", "driver_tag2", "driver_tag3", "driver_tag4", "driver_tag5"].forEach((tagField) => {
            const tag = Object.keys(item).find((f) => f.toLowerCase() === tagField.toLowerCase());
            if (tag && item[tag] && item[tag].trim()) {
                const tagElement = document.createElement("div");
                tagElement.className = "driver-tag";
                tagElement.textContent = item[tag].trim();
                driverTagsContainer.appendChild(tagElement);
            }
        });

        // Append tags container to item
        itemElement.appendChild(driverTagsContainer);

        // Create display actions container
        const displayActionsContainer = document.createElement("div");
        displayActionsContainer.className = "display-actions-container";

        // Add display dismiss button if exists
        const displayDismissField = Object.keys(item).find((f) => f.toLowerCase() === "displaydismiss");
        if (displayDismissField) {
            const dismissButton = document.createElement("div");
            dismissButton.className = `display-action display-action-${item[displayDismissField]}`;
            dismissButton.textContent = "Dismiss: " + item[displayDismissField].toString().toUpperCase();
            displayActionsContainer.appendChild(dismissButton);
        }

        // Add mark as complete button if exists
        const displayMarkAsCompleteField = Object.keys(item).find((f) => f.toLowerCase() === "displaymarkascomplete");
        if (displayMarkAsCompleteField) {
            const markAsCompleteButton = document.createElement("div");
            markAsCompleteButton.className = `display-action display-action-${item[displayMarkAsCompleteField]}`;
            markAsCompleteButton.textContent =
                "Mark as Complete: " + item[displayMarkAsCompleteField].toString().toUpperCase();
            displayActionsContainer.appendChild(markAsCompleteButton);
        }

        // Append display actions container to item
        itemElement.appendChild(displayActionsContainer);

        selectedFields.forEach((selectedField) => {
            // Skip driver tags and display actions as they're already handled
            if (
                selectedField.startsWith("driver_tag") ||
                selectedField.toLowerCase() === "displaydismiss" ||
                selectedField.toLowerCase() === "displaymarkascomplete"
            )
                return;

            // Find the actual field name with correct casing
            const field = Object.keys(item).find((f) => f.toLowerCase() === selectedField.toLowerCase());
